  ...
  bool cDaemon::is_continue(cMatch &match, const cMove &move, const uint8_t depth){
      ...
      // king attacking move
      if(depth <= maxdepth && cEvaluator::does_move_do_check(match, move)){
          //incr_tracer(depth, 3);
          if(move.is_capture()){
              m_capture_move_cnt++;
          }
      ...
      if(move.is_capture()){
          ...
          else if(m_last_move_was_capture){ // depth <= maxdepth || 
              if((m_capture_move_cnt + 1) <= 2){ 
                  //incr_tracer(depth, 5);
                  m_capture_move_cnt++;
      ...
      if(move.is_capture() == false && m_other_move_cnt == 0 && m_capture_move_cnt > 0){
          //incr_tracer(depth, 6);
          m_other_move_cnt++;
